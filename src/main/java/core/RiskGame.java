...
public class RiskGame extends JFrame implements MouseListener, MouseMotionListener{
    ...
    public void mousePressed(MouseEvent e) {
        // If the player has more than 5 cards, view them for potential cash-in
        ...
        if (isActive && !view) {
            if (selected && select != active && Countries.get(select).isNeighbor(Countries.get(active))) {
                Country attacker = Countries.get(select);
                Country defender = Countries.get(active);

                // Handling the attack scenario
                if (attack && defender.getPossession() != turn && attacker.getArmies() > 1) {
                    attacker.attack(defender);  // Perform the attack
                    conquered = false;  // Reset conquered status

                    if (defender.isEmpty()) {
                        // Display conquered message and handle defeated player's cards
                        JOptionPane.showMessageDialog(frame,
                                "You have conquered " + defender.getName() + "!",
                                "Information",
                                JOptionPane.INFORMATION_MESSAGE);
                        ...
                        if (!getCard) {
                            ...
                            Players.get(attacker.getPossession()).gotCard(Cards.get(temp));
                            Cards.remove(temp);
                        }

                        Player defeatedPlayer = Players.get(defender.getPossession());

                        if (defeatedPlayer.getCountries() == 0) {
                            // Transfer cards from the defeated player
                            Players.get(attacker.getPossession()).gotCards(defeatedPlayer);

                            JOptionPane.showMessageDialog(frame,
                                    Players.get(turn).getName() + " has defeated " + defeatedPlayer.getName() + "!",
                                    "Information",
                                    JOptionPane.INFORMATION_MESSAGE);
                        ...
                        // Update game state and handle troop transfer
                        Players.get(attacker.getPossession()).conquered(defender);
                        defeatedPlayer.lost(defender);
                        defender.conqueredBy(attacker.getPossession());
                ...
                // Fortification logic
                else if (fortify && defender.getPossession() == turn && attacker.isNeighbor(defender)) {
                    if (!fortified && attacker.getArmies() > 1) {
                        fortified = true;
                        fortify1 = select;
                        fortify2 = active;
                    }
                    ...
                    if ((fortify1 == select && fortify2 == active) || (fortify1 == active && fortify2 == select)) {
                        if (attacker.getArmies() > 1 && fortified) {
                            attacker.lose(1);
                            defender.gain(1);
                        }
                    }
            ...
            // Selecting a country for a turn
    ...
    private void handleCardSelection() {
        if (isActive && view) {
            ...
            if (cashIn == 1 && active != cash2 && active != cash3) {
                cash1 = active;
                cashIn++;
            } else if (cashIn == 2 && active != cash1 && active != cash3) {
                cash2 = active;
                cashIn++;
            } else if (cashIn == 3 && active != cash1 && active != cash2) {
                cash3 = active;
                cashIn++;
            }
        }
    ...
    public void mouseReleased(MouseEvent e) {
        // Logic for handling mouse release events
        handleMouseEvent(e);
    }

    public void mouseClicked(MouseEvent e) {
        // Logic for handling mouse click events
        handleMouseEvent(e);
    }

    public void mouseDragged(MouseEvent e) {
        // Logic for handling mouse drag events
        handleMouseEvent(e);
    }
    ...
    private void handleMouseEvent(MouseEvent e) {
        ...
        // Check if clicked on any of the custom buttons
        checkCustomButtons(x, y);

        Country clickedCountry = findTerritoryUnderMouse(x, y);

        if (clickedCountry != null) {
            handleCountryInteraction(clickedCountry);
        }
    }

    private Country findTerritoryUnderMouse(int x, int y) {
        for (Country country : Countries) {
            if (country.giveBorder().contains(x, y)) {
                return country;
            }
        }
        return null;
    ...
    private void checkCustomButtons(int x, int y) {
        if (Attack.contains(x, y)) {
            a = true; // Set attack state
        } else if (View.contains(x, y)) {
            view = true; // Set view state
        } else if (Fortify.contains(x, y)) {
            f = true; // Set fortify state
        } else if (End.contains(x, y)) {
            end = true; // End the turn
        } else if (Quit.contains(x, y)) {
            System.exit(0); // Quit the game
        }
    }

    private void handleCountryInteraction(Country clickedCountry) {
        Player currentPlayer = Players.get(turn);
        Country selectedCountry = this.selectedCountry;

        if (clickedCountry.getPossession() == currentPlayer.getPlayer()) {
            // If the player owns the clicked country, select it
            this.selectedCountry = clickedCountry;
        } else if (selectedCountry != null && selectedCountry.isNeighbor(clickedCountry)) {
            // Check for attack opportunity
            selectedCountry.attack(clickedCountry);

            // Check if the attack was successful
            if (clickedCountry.getArmies() == 0) {
                // Update possession and game state
                clickedCountry.conqueredBy(currentPlayer.getPlayer());
                currentPlayer.conquered(clickedCountry);

                // Check if the defeated player's countries are all conquered
                Player defeatedPlayer = Players.get(clickedCountry.getPossession());
                if (defeatedPlayer.getCountries() == 0) {
                    // Transfer cards from the defeated player
                    currentPlayer.gotCards(defeatedPlayer);
                    ...
                    if (currentPlayer.getCountries() == 48) {
                        JOptionPane.showMessageDialog(frame,
                                "Congratulations! You have conquered the world!",
                                "Information",
                                JOptionPane.INFORMATION_MESSAGE);
                    ...
                    // Prompt to cash in cards if needed
                    if (currentPlayer.getCardsN() > 5) {
                        JOptionPane.showMessageDialog(frame,
                                "You have more than 5 cards. Please cash in.",
                                "Information",
                                JOptionPane.INFORMATION_MESSAGE);
    ...
    public void postInfo(String text, Graphics2D graphics) {
        graphics.setFont(new Font("English", Font.PLAIN, 13));
